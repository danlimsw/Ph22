...
"""
Runge-Kutta ODE Solver (maximum 12 variables!)
Input:
...
Values vector length: %d' %(len(func),len(values))
    ...
    values = values + [0 for i in range(12-N)]
    values = values + [t]
    outlist = []
    for i in range(N):
        function = lambda x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,t:eval(func[i])
        out = function(*values)
        outlist = outlist + [out]
    return outlist
...
Initial condition value length: %d' %(len(func),len(init))
    ...
    savefile.write('t,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12\n')
    init = init + [0 for i in range(12-N)]
    func = func + ['0' for i in range(12-N)]
